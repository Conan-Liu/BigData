...
object CommonFriends {
    ...
    def main(args: Array[String]): Unit = {
        ...
        val comFri = reverseFriend1.flatMap(x => {
            ...
            /**
              * 正常来说，使用常规的两层for循环嵌套并没有什么问题，但是大多数嵌套循环的场景其实都是为了获取到内部元素
              * 而且嵌套的for容易导致金字塔式的代码，所以scala的嵌套语法糖从可读性上来说,是值得使用的。
              * 多层嵌套使用,内层的迭代元素可以同时取到外层循环的元素和内层循环的元素，生命周期和常规的嵌套for循环一致。
              * // 4. 嵌套for
