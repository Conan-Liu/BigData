...
public class MultipleOutputDir extends Configured implements Tool {
    ...
    public int run(String[] args) throws Exception {
        ...
        FileInputFormat.addInputPath(job, new Path("/user/hive/temp/in/"));
        FileOutputFormat.setOutputPath(job, out);
        return job.waitForCompletion(true) ? 0 : 1;
    }

    public static void main(String[] args) throws Exception {
        int result = ToolRunner.run(HadoopConf.getHAInstance(), new MultipleOutputDir(), args);
        System.exit(result);
    }
